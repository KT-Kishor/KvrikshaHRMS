    ...
    function (BaseController, JSONModel, MessageToast, utils, Currency, Formatter) {
        ...
        return BaseController.extend("sap.kt.com.minihrsolution.controller.CreditNoteDetails", {
            ...
            onChangeAddCustomer: async function (oEvent) {
                ...
                this.SelectKey = oSource.getSelectedKey();

                var SelectedData = oView.getModel("ManageCustomerModel")
                    .getData()
                    .filter(item => item.ID === this.SelectKey)[0];

                if (!SelectedData) return;

                this.getBusyDialog();

                // ✅ CLEAR INVOICE SELECTION (IMPORTANT FIX)
                this.SelectedCustomerModel.setProperty("/InvNo", "");
                this.byId("idMainInvoiceCombo").setSelectedKey("");
                this.byId("idMainInvoiceCombo").setValue("");
                ...
                this.SelectedCustomerModel.setProperty("/Address",
                    SelectedData.address + ", " + SelectedData.baseLocation + ", " + SelectedData.country
                );
                ...
                // Reset items
                oView.getModel("CompanyInvoiceItemModel").setProperty("/CompanyInvoiceItem", []);
                this.byId("CCI_id_SowDetails").setValue("").setSelectedKey("");

                this.SelectedCustomerModel.refresh(true);

                try {
                    const oAllInvoiceModel = oView.getModel("CompanyInvoiceData");
                    const aAllInvoices = oAllInvoiceModel ? oAllInvoiceModel.getData() : [];

                    const sSelectedCustomerName = SelectedData.companyName;

                    const aFilteredInvoices = aAllInvoices.filter(
                        inv => inv.CustomerName === sSelectedCustomerName
                    );

                    const oFilteredModel = new sap.ui.model.json.JSONModel({
                        InvoiceList: aFilteredInvoices
                    });

                    oView.setModel(oFilteredModel, "FilteredInvoiceModel");
                ...
                // Fetch combined data
                await this.ajaxCreateWithJQuery("combineSowMsa", { MsaID: this.SelectKey })
                    .then((oData) => {
                        if (oData.success) {
                            oView.getModel("CombinedData").setProperty("/items", oData.combinedData);
                            this.closeBusyDialog();
            ...
            onChangeInvoiceNo: function (oEvent) {
                ...
                const oSelectedCustomerModel = oView.getModel("SelectedCustomerModel");
                const aInvoices = oView.getModel("FilteredInvoiceModel").getProperty("/InvoiceList") || [];

                // Find selected invoice
                const oSelectedInvoice = aInvoices.find(inv => inv.InvNo === sSelectedKey);

                if (!oSelectedInvoice) return;

                // Set invoice number
                oSelectedCustomerModel.setProperty("/InvNo", sSelectedKey);

                // Store TotalAmount for validation
                oSelectedCustomerModel.setProperty("/SelectedInvoiceTotal", parseFloat(oSelectedInvoice.TotalAmount) || 0);

                oSelectedCustomerModel.setProperty("/InvoiceOriginalAmount", parseFloat(oSelectedInvoice.TotalAmount) || 0);
            ...
            totalAmountCalculation: function () {
                ...
                aSOWDetails.forEach((item) => {
                    ...
                    let finalItemAmount = baseAmount - discountAmount;
                ...
                // ----------- USE DIRECT FINAL AMOUNT -----------

                oSOWModel.setProperty("/TotalAmount", finalAmount);
                oSOWModel.setProperty("/gstAmount", gstAmount.toFixed(2));
                oCustomerModel.setProperty("/TotalAmount", finalAmount);
            ...
            CID_onPressback: function () {
                ...
                if (this.dash === "InvoiceDashboard") {
                    ...
                    this.getRouter().navTo("RouteCreditNote", {FileName: "CreditNoteDetails"});
                }
            },
            ...
            CID_onPressSubmit: async function (oEvent) {
                try {
                    ...
                    const oSelectedCustomerModel = this.getView().getModel("SelectedCustomerModel").getData();
                    ...
                    const fCreditNote = parseFloat(oModel.TotalAmount) || 0;
                    const fInvoiceTotal = parseFloat(oSelectedCustomerModel.SelectedInvoiceTotal) || 0;

                    // ✅ VALIDATION
                    if (fCreditNote > fInvoiceTotal) {
                        sap.m.MessageBox.error(
                            `Credit Note amount (${fCreditNote}) cannot exceed Invoice Total (${fInvoiceTotal})`
                        );
                        return;
                    }
                    ...
                    try {
                        ...
                        var oDialog = new sap.m.Dialog({
                            ...
                            beginButton: new sap.m.Button({
                                ...
                                press: function () {
                                    oDialog.close();
                                    that.getRouter().navTo("RouteCreditNote", { FileName: "CreditNoteDetails"});
                                }
                            }),
