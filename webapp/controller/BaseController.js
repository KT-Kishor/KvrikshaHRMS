...
], function (Controller, JSONModel, jsPDF, Formatter, BusyIndicator, CalendarLegend, CalendarLegendItem, DateTypeRange, utils, MessageToast, MessageBox) {
    "use strict";
    ...
    return Controller.extend("sap.kt.ktofferletter.products.controller.BaseController", {
        ...
        onSupportrequestChange: function (oEvent) {
            ...
            if (aAttachments.length + oFiles.length > 3) {
                return MessageToast.show("You can upload maximum 3 files only");
            }
        ...
        RB_onCancelButtonPress: function () {
            ...
            if (oBugModel) oBugModel.setData({
                AppName: "",
                BugDescription: "",
                Email: "",
                Save: false
    ...
    dialog.open();
        ...
        CommonVisitingCard: async function (EmployeeName, MobileNo, Email, Designation, branchCode) {
            ...
            try {
                ...
                doc.setTextColor("#FFFFFF");
                doc.textWithLink(companyurl, 13, 38, {
                    url: companyurl
                });
        ...
        initCalendarLegend: async function (oCalendar, sBranchCode) {
            ...
            const oLegend = new CalendarLegend({
                items: [
                    new CalendarLegendItem({
                        type: "Type04",
                        text: oI18nBundle.getText("calendarHoliday")
                    }),
                    new CalendarLegendItem({
                        type: "Type09",
                        text: oI18nBundle.getText("calendarWeekend")
                    }),
                    new CalendarLegendItem({
                        type: "Type06",
                        text: oI18nBundle.getText("calendarWorkingDay")
                    }),
                    new CalendarLegendItem({
                        type: "Type07",
                        text: oI18nBundle.getText("calendarFutureDate")
                    })
                ]
            });
            ...
            try {
                ...
                for (let d = new Date(yearStart); d <= yearEnd; d.setDate(d.getDate() + 1)) {
                    ...
                    const oDateRange = new DateTypeRange({
        ...
        TileNotificationReadData: async function () {
            ...
            await this._fetchCommonData("IncomeAsset", "Returntilemodel", { Status: "Return request", ReturnrequestEmpID: Loginmodel.EmployeeID });
        ...
        Tile_NotifictionBTN: async function (oEvent) {
            ...
            const oBusyModel = new sap.ui.model.json.JSONModel({
                busyIndicator: true
            });
            oView.setModel(oBusyModel, "setModelforBusy");
            ...
            if (!this.oDialog) {
                ...
                this.oDialog.attachAfterOpen(this.initializeBirthdayCarousel.bind(this));
            }
        ...
        onClosePopover: function () {
            if (this.oDialog) {
                this.oDialog.attachAfterClose(function () {
                    ...
                    this.oDialog = null; // IMPORTANT (so it reloads next time)
                }.bind(this));
        ...
        OnPressNavigationSOW: function (oEvent) {
            var MsaID = oEvent.getSource().getBindingContext("SOWModel").getProperty("MsaID");
            this.getRouter().navTo("RouteMSAEdit", {
                sPath: MsaID
            });
            this.onClosePopover();
