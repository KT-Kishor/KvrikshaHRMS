sap.ui.define(["./BaseController", "../utils/validation", "sap/ui/model/json/JSONModel", "../utils/CommonAgreementPDF", "sap/m/MessageToast", "../model/formatter"], function (BaseController, utils, JSONModel, jsPDF, MessageToast, Formatter) {
    ...
    return BaseController.extend("sap.kt.com.minihrsolution.controller.MSAEdit", {
        ...
        MsaE_GoToInvoice: function () {
            this.getRouter().navTo("RouteCompanyInvoice",{FileName: "MSAEdit"});
        },
        ...
        SOW_onCloseFrag: function () {
            ...
            sap.ui.getCore().byId("Sow_id_Currency").setValueState("None");
        ...
        MSA_onChangeCountry: function (oEvent) {
            ...
            const oCityCB = sap.ui.getCore().byId("MSA_Nav_Id_City");
            const oSTD = sap.ui.getCore().byId("Msa_LC_odeModel");
            const oMobile = sap.ui.getCore().byId("Msa_LC_Mobile");

            // Clear value state
            oCountry.setValueState("None");

            // Reset Model
            ["Country", "State", "City", "STDCode", "phone"].forEach(p => oModel.setProperty("/" + p, ""));

            // Reset UI
            oStateCB?.setSelectedKey("").setValue("");
            oCityCB?.setSelectedKey("").setValue("");
            oSTD?.setValue("");
            oMobile?.setValue("");
            ...
            if (oCountryData?.stdCode) {
                ...
                this.MSA_onChangeStdCode(); // same behavior as MC
            }

            // Filter States only if country code exists
            if (sCountryCode) {
                oStateCB?.getBinding("items")?.filter([
        ...
        MSA_onChangeStdCode: function () {
            ...
            this.MSA_validateMobile(oMobile);
        },

        MSA_validateMobile: function (oEventOrControl) {

            // Resolve control safely (event OR direct control)
            let oInput;
            if (oEventOrControl?.getSource instanceof Function) {
                oInput = oEventOrControl.getSource();
            } else if (oEventOrControl?.getValue instanceof Function) {
            ...
            if (std === "+91") {
                // India: exactly 10 digits, cannot start with 0
                isValid = /^[1-9]\d{9}$/.test(sValue);
                if (!isValid) {
                    oInput.setValueStateText("Mobile number must be 10 digits (India)");
                }
            } else {
                // International: 4–18 digits
                isValid = /^\d{4,18}$/.test(sValue);
                if (!isValid) {
                    oInput.setValueStateText("Enter valid international mobile number");
                }
            }
        ...
        MSA_onChangeState: function (oEvent) {
            ...
            // Filter cities only if country code is available
            if (sCountryCode) {
                oCityCB.getBinding("items")?.filter([
        ...
        MsaE_onEditOrSavePress: async function (oEvent) {

            // 🔹 Open custom busy dialog
            this.getBusyDialog();

            var sControlId = oEvent.getSource().getId().split("-").pop();
            ...
            try {
                ...
                if (sControlId === "RenewMSA") {
                    this.SimpleFormModel.setProperty("/EditMSA", false);
                    this.SimpleFormModel.setProperty("/RenewMSA", true);

                    var oModel = this.getView().getModel("FilteredMsaModel");
                    var aData = oModel.getData();

                    var sExtendedDate = aData[0].ExtendedDate;
                    this.ExtendedDate = sExtendedDate;

                    if (sExtendedDate) {
                        var aDates = sExtendedDate.split(",").map(function (sDate) {
                            return sDate.trim();
                        });
                        sLastDate = aDates[aDates.length - 1];
                    }

                } else {
                    this.SimpleFormModel.setProperty("/EditMSA", true);
                    this.SimpleFormModel.setProperty("/RenewMSA", false);
                }

                var type = this.getView().getModel("FilteredMsaModel").getData()[0].Type;

                this.SimpleFormModel.setProperty("/Recruitment", type === "Recruitment");

                // 🔹 Load Fragment
                if (!this.MSA_oDialog) {
                    this.MSA_oDialog = await sap.ui.core.Fragment.load({
                ...
                // 🔥 Close busy dialog BEFORE opening fragment
                this.closeBusyDialog();
                ...
                // 🔹 Set Values
                sap.ui.getCore().byId("MsaE_id_Type").setSelectedIndex(type === "Recruitment" ? 1 : 0);
                sap.ui.getCore().byId("id_MSA_ExtenedDate").setValue(sLastDate || "");

                this.resetMsaDialogFields();
                this._FragmentDatePickersReadOnly(["MsaE_id_CreateMSADate"]);
                this._applyCountryStateCityFilters();
                this.MSA_onChangeStdCode();
        ...
        MSA_Frg_Update: async function () {
            ...
            if (validationsPassed) {
                ...
                oModel.CreateMSADate = sap.ui.getCore().byId("MsaE_id_CreateMSADate").getValue().split("/").reverse().join("-");

                if (this.SimpleFormModel.getProperty("/RenewMSA") === true) oModel.ExtendedDate = this.ExtendedDate ? this.ExtendedDate + ", " + sap.ui.getCore().byId("id_MSA_ExtenedDate").getValue() : sap.ui.getCore().byId("id_MSA_ExtenedDate").getValue();
        ...
        SOW_onSaveFrag: async function () {
            if (utils._LCvalidateMandatoryField(sap.ui.getCore().byId("SOW_id_MsaDesc"), "ID") && utils._LCvalidateDate(sap.ui.getCore().byId("SOW_id_StartDate"), "ID") && utils._LCvalidateDate(sap.ui.getCore().byId("SOW_id_EndDate"), "ID") && utils._LCstrictValidationComboBox(sap.ui.getCore().byId("Sow_id_Currency"), "ID") && this.Rate && this.Desiganation && this.ConsultantName) {
        ...
        MsaE_onChangeCurrency: function (oEvent) {
        ...
        async MsaE_onPressMerge() {
            ...
            oPDFModel.setProperty("/ClientCompanyAddress", oModel.Address + ", " + oModel.City + ", " + oModel.Country);
