    ...
    function (BaseController, Formatter, MessageBox, jsPDF) {
        ...
        return BaseController.extend("sap.kt.com.minihrsolution.controller.AdminPaySlipDetails", {
                ...
                try {
                    if (this.oModel.getProperty("/isCreate")) {
                        ...
                        if (response.success) {
                            ...
                            let workingDays = 0;
                            let showWorkingDays = false;

                            if (oData.EarningData && oData.EarningData.length > 0) {
                                const variablePay = oData.EarningData.find(
                                    item => item.Description === "Variable Pay"
                                );

                                workingDays = variablePay?.WorkingDays || 0;

                                // Show only if meaningful value exists
                                if (workingDays > 0) {
                                    showWorkingDays = true;
                                }
                            }
                            oData.TotalWorkingDays = workingDays;
                            oData.ShowWorkingDays = showWorkingDays;
            ...
            APD_onPressSubmit: async function () {
                ...
                delete data.TotalWorkingDays;
                delete data.ShowWorkingDays;
