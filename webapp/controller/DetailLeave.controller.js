sap.ui.define([
    ...
    return BaseController.extend("sap.kt.com.minihrsolution.controller.DetailLeave", {
        ...
        _onRouteMatched: async function(oEvent) {
            ...
            const aLeaveTypes = this.getView().getModel("leaveTypeModel")?.getData() || [];
            ...
            let oType = "All In One Leave";
            let selectedType = 1;

            const oMatchedType = aLeaveTypes.find(item =>
                item.name === this.modelData.SubType
            );

            if (oMatchedType) {
                oType = oMatchedType.name;
                selectedType = parseInt(oMatchedType.id, 10);
            }

            this.getView().getModel("selectedModel").setProperty("/selectedType", selectedType);
