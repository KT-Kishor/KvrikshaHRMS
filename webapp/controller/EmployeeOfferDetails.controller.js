    ...
    function (BaseController, utils, JSONModel, MessageToast, MessageBox, jsPDF, Formatter) {
        ...
        return BaseController.extend("sap.kt.com.minihrsolution.controller.EmployeeOfferDetails", {
            ...
            _onRouteMatched: async function (oEvent) {
                ...
                this.byId("EOD_id_CompanyCode").setSelectedKey("");
            },
                ...
                if (oViewModel.getProperty("/editable")) {
                    ...
                    this.showConfirmationDialog(
                        ...
                        function () {
                            ...
                            this.getRouter().navTo("RouteEmployeeOffer", { valueEmp: "EmployeeOfferDetails", from: "EmployeeOfferDetails" });
                        }.bind(this),
                    );
                } else {
                    this.getRouter().navTo("RouteEmployeeOffer", { valueEmp: "EmployeeOfferDetails", from: "EmployeeOfferDetails" });
                                ...
                                var oDialog = new sap.m.Dialog({
                                    ...
                                    beginButton: new sap.m.Button({
                                        ...
                                        press: function () {
                                            ...
                                            // this.getView().getModel("CompanyCodeDetailsModel")?.refresh(true);
                                            this.getRouter().navTo("RouteEmployeeOffer", { valueEmp: "EmployeeOffer", from: "EmployeeOfferDetails" });
                                        }.bind(this)
                                    }),
            ...
            Mail_onSendEmail: function () {
                try {
                    ...
                    var oPayload = {
                        ...
                        "EmploymentBond": oModel.EmploymentBond
                    };
                    this.getBusyDialog();
            ...
            async offerGeneratingPdfFunction(oModel) {
                ...
                try {
                    ...
                    if (oEmpModel.EmploymentBond == "0" || !oEmpModel.EmploymentBond) {
                        ...
                        oPDFModel.setProperty("/BondCondition", oEmpModel.EmploymentBond + " employment year(s)");
            ...
            EOD_onChangeEmployee: function (oEvent) {
                var oSelectedItem = oEvent.getSource().getSelectedItem();
                if (!oSelectedItem) return;
                var sEmployeeID = oSelectedItem.getAdditionalText();

                var oEmpModel = this.getView().getModel("EmpDetails");
                if (!oEmpModel) return;

                var aEmployees = oEmpModel.getData() || [];

                var oEmployeeData = aEmployees.find(function (emp) {
                    return String(emp.EmployeeID) === String(sEmployeeID);
                });

                if (!oEmployeeData) return;

                var oModel = this.getView().getModel("employeeModel");

                oModel.setProperty("/Salutation", oEmployeeData.Salutation);
                oModel.setProperty("/ConsultantName", oEmployeeData.EmployeeName);
                oModel.setProperty("/Country", oEmployeeData.Country);
                oModel.setProperty("/CountryCode", oEmployeeData.CountryCode);
                oModel.setProperty("/State", oEmployeeData.State);
                oModel.setProperty("/BaseLocation", oEmployeeData.BaseLocation);
                oModel.setProperty("/EmployeeEmail", oEmployeeData.CompanyEmailID);
                oModel.setProperty("/ConsultantAddress", oEmployeeData.PermanentAddress);
                oModel.setProperty("/PinCode", oEmployeeData.PinCode);
            }
        });
    });