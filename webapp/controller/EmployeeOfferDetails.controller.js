<mvc:View controllerName="sap.kt.com.minihrsolution.controller.EmployeeOfferDetails"
	...
	<Page id="EOD_id_PageCrate" title="{i18n>headerEmployeeOffer}" showNavButton="true" navButtonPress="EOD_onPressBackBtn" class="service-background glassHeader ObjectcustomHeaderToolbar classheader">
		<headerContent>
			<Button press="Tile_NotifictionBTN" class="sapUiTinyMarginBeginEnd LogNot logoutBtn" tooltip="{i18n>TilenotificationfragName}" icon="sap-icon://bell" type="Transparent" visible="{= ${LoginModel>/Role} !== 'Contractor' }">
				<customData >
					<BadgeCustomData class="badgeClass" key="badge" value="{EndingSoonModel>/notificationCount}"/>
				</customData>
			</Button>
			<Button press="TP_onraisebugpress" icon="sap-icon://incident" tooltip="{i18n>raisebug}" type="Transparent" class="LogNot logoutBtn"/>
			<Button press="onLogout" icon="sap-icon://log" tooltip="{i18n>logoutTooltip}" type="Transparent" class="LogNot logoutBtn"/>
		</headerContent>
		<content >
			...
			singleContainerFullSize="false">
								<f:content>
									<Label text="Previous Employee"/>
									<ComboBox items="{path: 'EmpDetails>/',filters: [{path: 'EmployeeStatus',operator: 'EQ',value1: 'Inactive'}]}" change="EOD_onChangeEmployee" showSecondaryValues="true" filterSecondaryValues="true" selectedKey="{employeeModel>/PreviousEmployeeID}" >
										<core:ListItem key="{EmpDetails>EmployeeID}" text="{= ${EmpDetails>EmployeeID} + ' (' + ${EmpDetails>EmployeeName} + ')' }" additionalText="{EmpDetails>EmployeeID}" />
									</ComboBox>
									<Label text="{i18n>companyBranch}" required="true" />
									<ComboBox items="{path:'CompanyCodeDetailsModel>/',length:1000}" showSecondaryValues="true" value="{employeeModel>/CompanyCode}" id="EOD_id_CompanyCode" valueStateText="{i18n>selectBranch}" change="EOD_onCompanyCodeChange" filterSecondaryValues="true">
										<core:ListItem key="{CompanyCodeDetailsModel>companyCode}" text="{CompanyCodeDetailsModel>companyCode}" additionalText="{CompanyCodeDetailsModel>branchCode} , {CompanyCodeDetailsModel>branch}"  />
									</ComboBox>
									<Label text="{i18n>branch}"/>
									<ComboBox items="{path:'CompanyCodeDetailsModel>/',length:1000}" showSecondaryValues="true" filterSecondaryValues="true" selectedKey="{employeeModel>/Branch}" id="EOD_id_Branch" valueStateText="{i18n>selectBranch}" enabled="false">
										<core:ListItem key="{CompanyCodeDetailsModel>branch}" text="{CompanyCodeDetailsModel>branch}" additionalText="{CompanyCodeDetailsModel>companyName}"/>
									</ComboBox>
									...
									<ComboBox change="EOD_onChangeCountry" valueStateText="{i18n>expValStaCountry}" id="EOD_id_Country" items="{path:'CountryModel>/',length:1000}" selectedKey="{employeeModel>/Country}" showSecondaryValues="true" filterSecondaryValues="true">
									...
									<ComboBox change="EOD_onChangeState" valueStateText="{i18n>expValStaState}" id="EOD_id_State" items="{path:'StateModel>/',length:1000}" selectedKey="{employeeModel>/State}" showSecondaryValues="true" >
										<core:ListItem key="{StateModel>stateName}" text="{StateModel>stateName}"/>
									</ComboBox>
									<Label text="{i18n>city}" required="true" />
									<ComboBox items="{path:'CityModel>/',length:1000}" showSecondaryValues="true" filterSecondaryValues="true" value="{employeeModel>/CityModel}" id="EOD_id_Location" valueStateText="{i18n>cityValueText}" change="EOD_validateCombobox">
										<items>
									...
									<Select items="{path:'CurrencyModel>/',length:1000}" enabled="false" selectedKey="{employeeModel>/Currency}" id="EOD_id_Currency" width="100%" class="marginLeft">
									...
									<ComboBox  selectedKey="{employeeModel>/EmploymentBond}" items="{BondModel>/bond}" id="EOD_id_BondCombo" >
										...
										<l:BlockLayoutCell  width="2">
											<f:SimpleForm title="{i18n>monthly}" editable="false" layout="ColumnLayout" columnsM="1" columnsL="1" columnsXL="1">
												<f:content>
													...
													<Text text="{path: 'employeeModel>/EmployerPF', formatter: '.Formatter.YearlyToMontlyConv'}" visible="{= ${employeeModel>/EmployerPF} !== 0}" />
													...
													<Text text="{path:'employeeModel>/EmployeePF',formatter:'.Formatter.YearlyToMontlyConv'}" visible="{= ${employeeModel>/EmployeePF} !== 0}" />
													<Label text="{i18n>performanceTax}" />
													<Text text="{path:'employeeModel>/PT',formatter:'.Formatter.YearlyToMontlyConv'}"  visible="{= ${employeeModel>/PT}!== 0}"/>
													<Label text="{i18n>incomeTax}" />
													<Text text="{path:'employeeModel>/IncomeTax',formatter:'.Formatter.YearlyToMontlyConv'}" visible="{= ${employeeModel>/IncomeTax} !==0}"/>
													<Label text="{i18n>totalDeductionAmount}" class="boldBlackText" />
													<Text text="{path:'employeeModel>/TotalDeduction',formatter:'.Formatter.YearlyToMontlyConv'}"  visible="{= ${employeeModel>/TotalDeduction}!== 0}" />
													<Label text="{i18n>variablePayTotal}" class="boldBlackText" />
													<Text visible="{= ${employeeModel>/VariablePay} !== 0 }" text="{path:'employeeModel>/VariablePay',formatter:'.Formatter.YearlyToMontlyConv'}"/>
										...
										<l:BlockLayoutCell  width="2">
											<f:SimpleForm title="{i18n>yearly}" editable="false" layout="ColumnLayout" columnsM="1" columnsL="1" columnsXL="1">
												<f:content>
													...
													<Label text="{i18n>eplyrPF}" />
													<Text text="INR {path:'employeeModel>/EmployerPF',formatter: '.Formatter.fromatNumber'}" visible="{= ${employeeModel>/EmployerPF}!== 0}"/>
													<Label text="{i18n>medicalInsurance}" />
													...
													<core:Title text="{i18n>deduction}"/>
													<Label text="{i18n>providentFund}" />
													<Text text="INR {path:'employeeModel>/EmployeePF',formatter: '.Formatter.fromatNumber'}" visible="{= ${employeeModel>/EmployeePF}!== 0}"/>
													<Label text="{i18n>performanceTax}" />
													<Text text="INR {path:'employeeModel>/PT',formatter: '.Formatter.fromatNumber'}"  visible="{= ${employeeModel>/PT}!== 0}"/>
													<Label text="{i18n>incomeTax}" />
													<Text text="INR {path:'employeeModel>/IncomeTax',formatter: '.Formatter.fromatNumber'}" visible="{= ${employeeModel>/IncomeTax}!== 0}"/>
													<Label text="{i18n>totalDeductionAmount}" class="boldBlackText" />
													<Text text="INR {path:'employeeModel>/TotalDeduction',formatter: '.Formatter.fromatNumber'}"  visible="{= ${employeeModel>/TotalDeduction}!== 0}"/>
													<Label text="{i18n>variablePayTotal}" class="boldBlackText" />
													<Text visible="{= ${employeeModel>/VariablePay} !== 0 }" text="INR {path:'employeeModel>/VariablePay',formatter: '.Formatter.fromatNumber'}"/>
													<Label text="{i18n>grossPayTotal}" class="boldBlackText"/>
									...
									<l:BlockLayoutRow >
										<l:BlockLayoutCell width="2">
											...
											<FlexBox direction="Row" justifyContent="Start" wrap="Wrap">
												...
												<VBox class="summaryBox"  visible="{= ${employeeModel>/TotalDeduction}!== 0}">
												...
												<VBox class="summaryBox" visible="{= ${employeeModel>/VariablePay}!== 0}">
												...
												<VBox class="summaryBox" visible="{= ${employeeModel>/JoiningBonus} * 1 !== 0 }">
		...
		<footer>
			<Bar>
				...
				<contentRight>
					<Button text="{i18n>submit}" press="EOD_onSubmitData" id="EOD_id_Submit" type="Transparent"  enabled="false"/>
				</contentRight>
			</Bar>
		</footer>
	</Page>
