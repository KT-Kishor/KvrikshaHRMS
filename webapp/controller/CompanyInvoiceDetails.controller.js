    ...
    function (BaseController, JSONModel, MessageToast, utils, Currency, Formatter) {
        ...
        return BaseController.extend("sap.kt.com.minihrsolution.controller.CompanyInvoiceDetails", {
            ...
            onChangeAddCustomer: async function (oEvent) {
                ...
                this.SelectedCustomerModel.setProperty("/MailID", SelectedData.customerEmail);
            ...
            totalAmountCalculation: function () {
                ...
                if (currency === "INR") {
                    if (gstType === "CGST/SGST") {
                        ...
                        finalNormal += (parseFloat(cgst) + parseFloat(sgst));
                    } else if (gstType === "IGST") {
                        gstAmount = (subTotalGST * gstRate) / 100;
                        igst = gstAmount;
                        finalNormal += parseFloat(igst);
                    }
                }

                finalNormal = parseFloat(finalNormal.toFixed(2));

                // ---------- COMMENT ROUND OFF CALCULATION ----------
                // const roundedNormal = Math.round(finalNormal);        // COMMENT THIS LINE
                // const roundOffNormal = (roundedNormal - Math.floor(finalNormal * 100) / 100).toFixed(2); // COMMENT
                // const RoundOff = roundOffNormal > 0 ? `+${roundOffNormal}` : roundOffNormal; // COMMENT

                const roundedNormal = finalNormal; // use actual value without rounding
                ...
                if (creditTaxable > 0 && currency === "INR") {
                    if (gstType === "CGST/SGST") {
                        ...
                        creditCGST = creditGSTAmt.toFixed(2);
                        creditSGST = creditGSTAmt.toFixed(2);
                        creditFinal += (parseFloat(creditCGST) + parseFloat(creditSGST));
                    } else if (gstType === "IGST") {
                        creditGSTAmt = (creditTaxable * gstRate) / 100;
                        creditIGST = creditGSTAmt.toFixed(2);
                        creditFinal += parseFloat(creditIGST);
                    }
                }

                creditFinal = parseFloat(creditFinal.toFixed(2));

                // ---------- COMMENT CREDIT ROUND OFF ----------
                // const roundedCredit = Math.round(creditFinal); // COMMENT THIS LINE
                // const creditRoundOff = (roundedCredit - Math.floor(creditFinal * 100) / 100).toFixed(2); // COMMENT
                // const CreditRoundOff = creditRoundOff > 0 ? `+${creditRoundOff}` : creditRoundOff; // COMMENT

                const roundedCredit = creditFinal; // without rounding
                ...
                oCustomerModel.setProperty("/IGST", igst);

                // oCustomerModel.setProperty("/RoundOff", RoundOff); // COMMENT THIS LINE

                oCustomerModel.setProperty("/CreditMemo", creditTaxable.toFixed(2));
                oCustomerModel.setProperty("/CreditCGST", creditCGST);
                oCustomerModel.setProperty("/CreditSGST", creditSGST);
                oCustomerModel.setProperty("/CreditIGST", creditIGST);
                ...
                // oSOWModel.setProperty("/RoundOf", RoundOff); // COMMENT THIS LINE
            ...
            Comp_OnChangeDiscount: async function (oEvent) {
                ...
                if (isPercentage) sValue = sValue.replace('%', '');
            ...
            CID_onPressAddCustomer: function () {
                ...
                this.getRouter().navTo("RouteManageCustomer", { value: "Company Invoice" });
            },

            CID_onPressMSAandSOW: function () {
                this.getView().getModel("LoginModel").setProperty("/RichText", false);
                (this.SelectKey) ? this.getRouter().navTo("RouteMSAEdit", { sPath: this.SelectKey }) : this.getRouter().navTo("RouteMSA");
            },
            ...
            onPressUpdateInvoice: async function () {
                try {
                    ...
                    this.onIncomeTaxPercentageInputLiveChange();
                    this.getBusyDialog();
                    ...
                    try {
                        ...
                        if (Status === "Payment Received" || Status === "Cancelled") {
                            this.visiablityPlay.setProperty("/MultiEmail", false);
                            this.visiablityPlay.setProperty("/Edit", false);
                        }
            ...
            CID_onPressSendMultipalEmail: function () {
                ...
                var oUploaderDataModel = new JSONModel({
                    ...
                    <ul style="margin-top: 0;">
                        ...
                        <li>Please note that payment is required to be made within the “Pay By” date mentioned in the invoice. If payment is not received within this period, an interest of 10% per month will be charged after a grace period of 15 days.</li>
                    </ul>
            ...
            onIncomeTaxPercentageInputLiveChange: function (oEvent) {
                if (oEvent) utils._LCvalidateVariablePay(oEvent);
                const oNavigationModel = this.getView().getModel("SelectedCustomerModel");

                if (oNavigationModel.getProperty("/Currency") === "INR") {
                    const subTotalInGST = parseFloat(oNavigationModel.getProperty("/SubTotalInGST")) || 0;
                    const subTotalNotGST = parseFloat(oNavigationModel.getProperty("/SubTotalNotGST")) || 0;
                    const incomePerc = parseFloat(oNavigationModel.getProperty("/IncomePerc")) || 0;
            ...
            CID_onPressGeneratePdf: async function () {
                ...
                try {
                    ...
                    let totalInWords = await this.convertNumberToWords(oModel.Total, data.Currency);
                    ...
                    detailsTable.forEach(row => {
                        ...
                        doc.setFont("times", "bold");
                        const valueX = rightAlignX + columnWidths[0] + 5;
                        const maxValueWidth = 30;
                        const valueLines = doc.splitTextToSize(row.value, maxValueWidth);
                        doc.text(valueLines, valueX, currentY + 5);
                        currentY += valueLines.length * rowHeight;
                    });
                    ...
                    const body = oCompanyItemModel.map((item, index) => {
                        ...
                        const particularsText =
                            item.Status === "Used" && item.CCInvNo ?
                                `${item.Particulars} (${item.CCInvNo})` :
                                item.Particulars;
                    ...
                    const percentageText =
                        oModel.Value !== undefined && oModel.Value > 0 ?
                            `(${oModel.Value}%)` :
                            "";
                    ...
                    // ================= CREDIT DETAILS =================
                    const CreditMemo = parseFloat(oModel.CreditMemo) || 0;
                    const creditCGST = parseFloat(oModel.CreditCGST) || 0;
                    const creditSGST = parseFloat(oModel.CreditSGST) || 0;
                    const creditIGST = parseFloat(oModel.CreditIGST) || 0;
                    // const creditRoundOff = parseFloat(oModel.CreditRoundOff) || 0;
                    const creditTotal = parseFloat(oModel.CreditMemoTotal) || 0;

                    if (CreditMemo > 0) {
                        summaryBody.push([
                            `Credit Note (${data.Currency}) :`,
                            Formatter.fromatNumber(CreditMemo.toFixed(2))
                        ]);
                    }

                    if (creditCGST > 0) {
                        summaryBody.push([
                            `Credit CGST (${data.Currency}) :`,
                            Formatter.fromatNumber(creditCGST.toFixed(2))
                        ]);
                    }

                    if (creditSGST > 0) {
                        summaryBody.push([
                            `Credit SGST (${data.Currency}) :`,
                            Formatter.fromatNumber(creditSGST.toFixed(2))
                        ]);
                    }

                    if (creditIGST > 0) {
                        summaryBody.push([
                            `Credit IGST (${data.Currency}) :`,
                            Formatter.fromatNumber(creditIGST.toFixed(2))
                        ]);
                    }

                    // if (creditRoundOff !== 0) {
                    //     summaryBody.push([
                    //         `Credit Round Off (${data.Currency}) :`,
                    //         Formatter.fromatNumber(creditRoundOff.toFixed(2))
                    //     ]);
                    // }

                    if (creditTotal > 0) {
                        summaryBody.push([
                            `Credit Total (${data.Currency}) :`,
                            Formatter.fromatNumber(creditTotal.toFixed(2))
                        ]);
                    }


                    const totalRowIndex = summaryBody.length;
                    summaryBody.push([`Total (${data.Currency}) :`, Formatter.fromatNumber(parseFloat(oModel.Total))]);
