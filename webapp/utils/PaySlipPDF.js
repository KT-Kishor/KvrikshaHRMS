sap.ui.define(["../model/formatter"], function (formatter) {
    ...
    return {
        ...
        // Added 'bBatchMode' parameter
        _GeneratePDF: function (that, oModel, oCompanyModel, bBatchMode) {
            ...
            doc.autoTable({
                ...
                body: earningData.map((item, index) => [
                    item.Description || "",
                    formatter.formatAmount(item.Amount ?? 0),
                    formatter.formatAmount(item.YearlyAmount ?? 0),
                    deductionData[index]?.Description || "",
                    formatter.formatAmount(deductionData[index]?.Amount ?? 0),
                    formatter.formatAmount(deductionData[index]?.YearlyAmount !== undefined ? deductionData[index].YearlyAmount : " ")
                ]).concat([
                    [
                        { content: "Total", styles: { fontStyle: "bold" } },
                        { content: formatter.formatAmount(oModel.EarningsTotalMonthly), styles: { fontStyle: "bold" } },
                        { content: formatter.formatAmount(oModel.EarningsTotalYearly), styles: { fontStyle: "bold" } },
                        { content: "Total", styles: { fontStyle: "bold" } },
                        { content: formatter.formatAmount(oModel.DeductionsTotalMonthly), styles: { fontStyle: "bold" } },
                        { content: formatter.formatAmount(oModel.DeductionsTotalYearly), styles: { fontStyle: "bold" } },
                    ]
                ]),
            ...
            doc.text(`Net Pay: Rs. ${formatter.formatAmount(oModel.NetPay)}`, margin + 2, yPosition);
            ...
            // --- MODIFIED CONDITION ---
            // Return blob if it's a batch process, otherwise prompt download as usual
            if (bBatchMode) {
                return doc.output("blob");
            } else {
                doc.save(`${oModel.EmployeeName} ${month}-${year} Pay Slip.pdf`);
                if (that && typeof that.closeBusyDialog === "function") {
