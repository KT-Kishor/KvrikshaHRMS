sap.ui.define(["../fonts/Montserrat"], function(Montserrat) {
    ...
    // Injects @font-face rules using the same base64 data registered on jsPDF,
    // so html2canvas rasterizes the real font instead of a fallback.
    function _ensureMontserratFontFace() {
        ...
        oStyle.textContent = "@font-face{font-family:'Montserrat';font-weight:normal;font-style:normal;" +
            "src:url(data:font/truetype;base64," + Montserrat.regular.base64 + ") format('truetype');}" +
            "@font-face{font-family:'Montserrat';font-weight:bold;font-style:normal;" +
            "src:url(data:font/truetype;base64," + Montserrat.bold.base64 + ") format('truetype');}";
    ...
    function _drawCircularWatermark(oPdf, sImgSrc, nPageWidth, nPageHeight, nWidthMm, nHeightMm, nOpacity) {
        ...
        try {
            ...
            oPdf.setGState(new oPdf.GState({ opacity: nOpacity }));
    ...
    return {
        /**
         * @param {object} mData - { referenceNumber, date, subject, content, includeSignature }
         * @param {object} [mCompanyInfo] - { companyName, address, logo, signature, backgroundLogo }
         *        Pass a background-free logo (e.g. transparentComplogo) - a baked-in
         *        border cannot be removed via CSS. backgroundLogo drives the watermark only.
         */
        generatePDF: function(mData, mCompanyInfo) {
            ...
            return new Promise(function(resolve, reject) {
                try {
                    ...
                    oContainer.style.fontFamily = "Montserrat";
                    oContainer.style.boxSizing = "border-box";

                    var sLogoHtml = sLogoSrc ?
                        '<img src="' + sLogoSrc + '" style="width:125px; height:125px; object-fit:contain; border:none; outline:none; box-shadow:none; background:transparent; display:block;" />' : '';

                    oContainer.innerHTML =
                        '<div style="width:100%; position:relative;">' +
                        '<div style="position:relative; z-index:1; padding:35px ' + nSidePaddingPx + 'px;">' +
                        '<div style="display:flex; align-items:flex-start; gap:16px; margin-bottom:4px;">' + sLogoHtml +
                        '<div><div style="font-family:Montserrat; font-size:32px; font-weight:bold; transform:scaleY(1.5); text-transform:uppercase; margin-top:18px; color:#1976D2;">' + sCompanyName + '</div>' +
                        '<div style="font-family:Montserrat; font-size:18px; line-height:1.5; margin-top:4px; color:#333; text-align:center; width:500px; word-wrap:break-word; white-space:normal;">' + sCompanyAddress + '</div></div></div>' +
                        '<div style="border-top:1.5px solid #333; margin: 5px 0 20px 0;"></div>' +
                        '<div style="display:flex; justify-content:space-between; margin-bottom:22px; font-size:18px;">' +
                        '<div>Ref No: ' + (mData.referenceNumber || "-") + '</div><div>Date: ' + (mData.date || "-") + '</div></div>' +
                        '<div style="margin-top:34px;"></div>' +
                        '<div style="text-align:center; font-size:18px; text-transform:uppercase; margin-bottom:22px;">' + (mData.subject || "") + '</div>' +
                        '<div style="font-size:18px; line-height:1.6; text-align:justify; margin-bottom:70px;">' + (mData.content || "") + '</div>' +
                        '</div></div>';
                    ...
                    var fRender = function() {
                        window.html2canvas(oContainer, {
                            ...
                            oPdf.setFont("Montserrat");
                            oPdf.text("Director", nFooterLeft, nFooterY + 24);

                            oPdf.save((mData.referenceNumber || "Document") + ".pdf");
                            resolve(oPdf);
